...
BEGIN
    ...
    -------------------------------
    INSERT INTO dbo.quality_check_log (check_name, check_description, check_result, issue_count, severity)
    SELECT 'CRM_CUST_INFO_MARITAL_STATUS', 'List of distinct cst_marital_status for review', 'INFO', COUNT(DISTINCT cst_marital_status), 'LOW'
    FROM silver.crm_cust_info;
    ...
    INSERT INTO dbo.quality_check_log (check_name, check_description, check_result, issue_count, severity)
    SELECT 'CRM_PRD_INFO_TRIM', 'Check for unwanted spaces in prd_nm',
    ...
    INSERT INTO dbo.quality_check_log (check_name, check_description, check_result, issue_count, severity)
    SELECT 'CRM_PRD_INFO_COST', 'Check for NULL or negative prd_cost',
           CASE WHEN COUNT(*) = 0 THEN 'PASS' ELSE 'FAIL' END, COUNT(*), 'HIGH'
    FROM bad_costs;
    ...
    -------------------------------
    INSERT INTO dbo.quality_check_log (check_name, check_description, check_result, issue_count, severity)
    SELECT 'CRM_PRD_INFO_LINE', 'List of distinct prd_line for review', 'INFO', COUNT(DISTINCT prd_line), 'LOW'
    ...
    INSERT INTO dbo.quality_check_log (check_name, check_description, check_result, issue_count, severity)
    SELECT 'CRM_PRD_INFO_DATE_ORDER', 'Check for prd_end_dt earlier than prd_start_dt',
    ...
    INSERT INTO dbo.quality_check_log (check_name, check_description, check_result, issue_count, severity)
    SELECT 'CRM_SALES_DETAILS_DUE_DT_FORMAT', 'Check for invalid due date format/range',
           CASE WHEN COUNT(*) = 0 THEN 'PASS' ELSE 'FAIL' END, COUNT(*), 'HIGH'
    FROM bad_due_dt;
    ...
    INSERT INTO dbo.quality_check_log (check_name, check_description, check_result, issue_count, severity)
    SELECT 'CRM_SALES_DETAILS_DATE_ORDER', 'Check for order date after shipping/due date',
           CASE WHEN COUNT(*) = 0 THEN 'PASS' ELSE 'FAIL' END, COUNT(*), 'MEDIUM'
    FROM bad_sales_dt;
    ...
    INSERT INTO dbo.quality_check_log (check_name, check_description, check_result, issue_count, severity)
    SELECT 'CRM_SALES_DETAILS_CONSISTENCY', 'Check that sales = quantity * price and values are positive/non-null',
           CASE WHEN COUNT(*) = 0 THEN 'PASS' ELSE 'FAIL' END, COUNT(*), 'HIGH'
    FROM bad_sales;
    ...
    INSERT INTO dbo.quality_check_log (check_name, check_description, check_result, issue_count, severity)
    SELECT 'ERP_CUST_AZ12_BIRTHDATE', 'Check for invalid birthdate range (1924-01-01 to Today)',
           CASE WHEN COUNT(*) = 0 THEN 'PASS' ELSE 'FAIL' END, COUNT(*), 'MEDIUM'
    FROM bad_bdates;
    ...
    -------------------------------
    INSERT INTO dbo.quality_check_log (check_name, check_description, check_result, issue_count, severity)
    SELECT 'ERP_CUST_AZ12_GEN', 'List of distinct gen values for review', 'INFO', COUNT(DISTINCT gen), 'LOW'
    ...
    -------------------------------
    INSERT INTO dbo.quality_check_log (check_name, check_description, check_result, issue_count, severity)
    SELECT 'ERP_LOC_A101_CNTRY', 'List of distinct cntry values for review', 'INFO', COUNT(DISTINCT cntry), 'LOW'
    ...
    INSERT INTO dbo.quality_check_log (check_name, check_description, check_result, issue_count, severity)
    SELECT 'ERP_PX_CAT_TRIM', 'Check for unwanted spaces in cat/subcat/maintenance',
           CASE WHEN COUNT(*) = 0 THEN 'PASS' ELSE 'FAIL' END, COUNT(*), 'LOW'
    FROM bad_cat_trim;
    ...
    -------------------------------
    INSERT INTO dbo.quality_check_log (check_name, check_description, check_result, issue_count, severity)
    SELECT 'ERP_PX_CAT_MAINTENANCE', 'List of distinct maintenance values for review', 'INFO', COUNT(DISTINCT maintenance), 'LOW'
